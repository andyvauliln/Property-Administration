...
def replaceText(payment: Payment, document_id, docs_service):
    ...
    rent_period = f"{payment.payment_date.strftime('%B %d %Y')} - {(payment.payment_date + relativedelta(months=1) ).strftime('%B %d %Y')}"
    current_date = payment.payment_date.strftime('%B %d %Y')
